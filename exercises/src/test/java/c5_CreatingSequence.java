...
public class c5_CreatingSequence {
    ...
    public void generate_programmatically() {
        AtomicInteger genCounter = new AtomicInteger(0);

        // emitts signals one-by-one via a consumer callback

        Flux<Integer> generateFlux = Flux.generate(sink -> {
            sink.next(genCounter.incrementAndGet());
            if (genCounter.get() == 5) {
                sink.complete();
            }
        });
        ...
        //------------------------------------------------------
        // emitts elements from different threads
        ...
        //------------------------------------------------------
        // emitts multiple elements from a single-thread
